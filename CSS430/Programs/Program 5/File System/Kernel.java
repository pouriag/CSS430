            ...
            switch( cmd ) { 
                  ...
                  switch ( param ) {
                     ...
                     default:
                        if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
                           ...
                           if(ftEnt == null) {
                              //System.err.println( "threaOS: ftEnt does not exist in ftEnt array" );
                              return ERROR;
                           }
               ...
               case OPEN:    // to be implemented in project
                  if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
                     ...
                     FileTableEntry ftEnt = fs.open(values[0], values[1]);
                     int fd = myTcb.getFd(ftEnt);
                     if(fd == -1) {
                        //System.err.println( "threaOS: filetableentry object is null or fdtable is full" );
                        return ERROR;
                     }
                     return fd;
                  }
               ...
               case CLOSE:   // to be implemented in project
                  if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
                     ...
                     if(ftEnt == null) {
                        //System.err.println( "threaOS: fd does not exist in ftEnt array" );
                        return ERROR;
                     }
                     if(fs.close(ftEnt)) {
                        return OK;
               ...
               if ( ( myTcb = scheduler.getMyTcb( ) ) != null ) {
                  int values[] = (int[])args;
                  FileTableEntry ftEnt = myTcb.getFtEnt(values[0]);

                  if(ftEnt == null) {
                     //System.err.println( "threaOS: fd does not exist in ftEnt array" );
                     return ERROR;
                  }
                  return fs.seek(ftEnt, values[1], values[2]);
               }
               return ERROR;
               case FORMAT:  // to be implemented in project
                  if(!fs.format(param)) {
                     //System.err.println("threadOS: cannot format, filetable already exists?");
                     return ERROR;
                  }
                  return OK;
